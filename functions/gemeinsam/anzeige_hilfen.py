"""Gemeinsame fachliche Anzeige- und Formatierungshilfen der App."""
...
def formatiere_zeitpunkt(wert: datetime | None) -> str:
    """Formatiert einen Zeitpunkt fuer die Anzeige."""
    if wert is None:
        return "-"
...
def formatiere_text(wert: str | None) -> str:
    """Bereinigt optionalen Text fuer die Anzeige."""
    if wert is None:
        return "-"
...
def formatiere_patient_label(patient: Patient) -> str:
    """Erzeugt ein kompaktes Anzeige-Label fuer einen Patienten."""
    return (
...
def hole_aktuellen_user_id() -> str:
    """Liest die aktuelle User-ID aus dem Session State."""
...
def loese_materialtyp_label_auf(materialtyp_code: str | None) -> str:
    """Loest einen Materialtyp-Code in das passende Label auf."""
...
def loese_analyse_label_auf(analyse_code: str | None) -> str:
    """Loest einen Analyse-Code in das passende Label auf."""
...
def loese_klinische_frage_label_auf(klinische_frage_code: str | None) -> str:
    """Loest den gespeicherten Code der klinischen Frage als Label auf."""
